...
public class MemberSelectListActivity extends AppCompatActivity {
    ...
    private ITuyaZigBeeLock iTuyaZigBeeLock;
    ...
    private void getProLockMemberList() {
        iTuyaZigBeeLock.getMemberList(new ITuyaDataCallback<ArrayList<MemberInfoBean>>() {
            @Override
    ...
    private void allocProUnlockOpMode(MemberInfoBean memberInfoBean) {
        ...
        if (from == 1) {
            iTuyaZigBeeLock.bindOpModeToMember(memberInfoBean.getUserId(), unlockIds, new ITuyaResultCallback<Boolean>() {
                @Override
        ...
        iTuyaZigBeeLock.allocUnlockOpMode(memberInfoBean.getUserId(), unlockIds, new ITuyaResultCallback<Boolean>() {
            @Override
            public void onSuccess(Boolean result) {
                Toast.makeText(getApplicationContext(), "alloc onSuccess", Toast.LENGTH_SHORT).show();
                finish();
